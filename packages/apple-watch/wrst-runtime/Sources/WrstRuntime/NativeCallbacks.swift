...
// Companion: sync snapshot read at startup (malloc'd JSON, caller frees) + send.
@_cdecl("swift_native_companion_status")
public nonisolated func swift_native_companion_status() -> UnsafeMutablePointer<CChar>? {
    let json: String = MainActor.assumeIsolated { CompanionManager.shared.statusJSON() }
    return strdup(json)
}

@_cdecl("swift_native_companion_send")
public nonisolated func swift_native_companion_send(_ json: UnsafePointer<CChar>?) {
    guard let json else { return }
    let s = String(cString: json)
    MainActor.assumeIsolated { CompanionManager.shared.send(s) }
}
