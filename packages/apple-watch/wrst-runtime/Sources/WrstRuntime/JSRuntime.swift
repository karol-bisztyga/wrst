...
final class JSRuntime {
    ...
    // exists if the app imports `Companion`. statusJSON is a JSON string the JS
    // side parses; its setState/recompute re-renders any views reading the cells.
    func companionStatus(_ statusJSON: String) {
        guard let bridge else { return }
        _ = qjs_bridge_eval(bridge, "globalThis.__wrstCompanionStatus && globalThis.__wrstCompanionStatus(\(Self.jsString(statusJSON)))", nil)
    }

    // Deliver an incoming message from the phone to JS onMessage handlers.
    func companionMessage(_ messageJSON: String) {
        guard let bridge else { return }
        _ = qjs_bridge_eval(bridge, "globalThis.__wrstCompanionMessage && globalThis.__wrstCompanionMessage(\(Self.jsString(messageJSON)))", nil)
    }

    // Encode a Swift string as a JS string literal (quoted + fully escaped) so it
    // can be spliced into an eval'd call argument.
