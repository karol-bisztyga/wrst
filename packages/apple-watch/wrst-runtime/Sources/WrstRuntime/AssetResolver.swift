...
enum AssetSource {
    ...
    case bundled(String)    // a resource embedded by `wrst build:apple-watch` (release)
    case none
}

enum AssetResolver {
    // - full URL (contains "://")        → used as-is (remote image)
    // - project-local name, debug        → http://localhost:8091/assets/<name> (dev server)
    ...
    static func resolve(_ src: String) -> AssetSource {
        ...
        #if DEBUG
        return URL(string: "http://localhost:8091/assets/\(src)").map { .url($0) } ?? .none
        #else
        return .bundled(src)
        #endif
    }
